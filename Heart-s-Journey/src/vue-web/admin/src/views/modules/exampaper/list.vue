		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #fff","padding":"10px","margin":"0","flexWrap":"wrap","background":"#ffffff","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">心理测试名称</label>
						<el-input v-model="searchForm.name" placeholder="心理测试名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
			...
			<div :style='{"border":"0px solid #fff","width":"100%","padding":"0 20px 20px","fontSize":"14px","color":"#000","background":"#fff"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="status" label="心理测试状态">
						<template slot-scope="scope">
							<el-switch
		...
		methods: {
			...
			composeSave() {
				this.$http({
					...
					&&radioNum=${this.composeForm.radioNum}&&multipleChoiceNum=0&&determineNum=0
					&&fillNum=0&&subjectivityNum=${this.composeForm.subjectivityNum}`,
					method: "post"
				}).then(({data})=>{
					if(data &&data.code==0){
						this.$message({
