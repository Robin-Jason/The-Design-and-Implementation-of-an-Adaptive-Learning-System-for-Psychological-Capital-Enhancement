	...
	<div class="container">
		...
		<el-card class="card_view" v-if="isEndFlag">
			...
			<div class="item-content" style="text-align: center;">
				<span class="label">
					您的心理：
					<span style="font-size:24px;font-weight:blod;color:red">{{determineForm.determine}}</span>
				</span>
			</div>
			<div class="item-content" style="text-align: center;width: 60%;">
				<span class="label">
					分析：
					<span>{{determineForm.analysis}}</span>
				</span>
			</div>
		...
		<el-card class="card_view" v-if="!isEndFlag||isSubmitFlag">
			<div v-for="(item,index) in dataList" :key="index" style="width: 100%;">
				<div class="item-content">
					...
					<span class="content ql-snow ql-editor" v-html="item.questionname"></span> {{item.type==4?`(${item.score}分)`:''}}
					<el-tag type="success" v-if="item.type==0">客观题</el-tag>
				...
				this.$confirm(`是否完成作答，提交心理测试？`, "提示", {
					...
					for(let x in this.dataList){
						if(this.dataList[x].type!=4){
							for(let i in this.dataList[x].optionList){
								if(this.dataList[x].myanswer == this.dataList[x].optionList[i].code){
									this.dataList[x].myscore = this.dataList[x].optionList[i].score
								}
							}
						}else{
