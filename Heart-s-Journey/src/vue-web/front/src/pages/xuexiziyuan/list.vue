			...
			<div class="list">
				...
				<div class="list1 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						...
						<div class="name">{{item.fabushijian}}</div>
						<div class="time_item">
...
<script>
	export default {
		...
		methods: {
			...
			async getFenlei() {
				await this.$http.get('ziyuanleixing/list',{params: {sort: 'id',order: 'asc'}}).then(res => {
					if (res.data.code == 0) {
						this.fenlei = res.data.data.list
					}
				});
				this.feileiColumn = 'ziyuanleixing'
			},
			getList(page, fenlei, ref = '') {
				...
				if (this.curFenlei != '全部') searchWhere.ziyuanleixing = this.curFenlei;
